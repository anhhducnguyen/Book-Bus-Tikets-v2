...
import { StationRepository } from "@/api/stations/stationRepository";
	...
	async findAll(): Promise<ServiceResponse<Station[] | null>> {
		try {
			...
			if (!stations || stations.length === 0) {
				return ServiceResponse.failure("No Station found", null, StatusCodes.NOT_FOUND);
			}
			return ServiceResponse.success<Station[]>("Stations found", stations);
		} catch (ex) {
			const errorMessage = `Error finding all station: $${(ex as Error).message}`;
